    ...
    public class GTFSRealtimeRetriever : BackgroundService
    {
        ...
        private async Task ProcessAlertUpdateAsync(RealTimeFeed feed, NpgsqlConnection dbConnection, IEnumerable<FeedEntity> alertUpdates)
        {
            ...
            var updatesArray = alertUpdates.Select(tripUpdate => new PsqlAlertUpdate
            {
                ...
                InternalId = Guid.NewGuid(),
                LastUpdated = DateTimeOffset.UtcNow,
                Effect = tripUpdate.Alert?.effect ?? Alert.Effect.UnknownEffect,
                Cause = tripUpdate.Alert?.cause ?? Alert.Cause.UnknownCause,
                SeverityLevel = tripUpdate.Alert?.severity_level ?? Alert.SeverityLevel.UnknownSeverity,
                Url = tripUpdate.Alert?.Url?.ToString(),
                HeaderText = tripUpdate.Alert?.HeaderText?.ToString(),
                DescriptionText = tripUpdate.Alert?.DescriptionText?.ToString()
            }).ToArray();

            // Insert PsqlAlertUpdate array using a stored procedure
            var alertUpdateCommand = new NpgsqlCommand("CALL public.upsert_trip_update_array(@updates)", dbConnection)
            {
                CommandType = CommandType.Text,
                Transaction = transaction
            };
            ...
            foreach (var tripUpdate in alertUpdates)
            {
                if (tripUpdate.Alert?.InformedEntities?.Count > 0)
                {
                    await InformEntitiesAsync(feed, tripUpdate.Alert.InformedEntities, dbConnection);
                }
            }
